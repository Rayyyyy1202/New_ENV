...
text(s, Inches(1.0), Inches(2.3), Inches(11), Inches(0.5),
     [[("AI 驱动 · 不用 RPA · 第三方数据 + 大模型", 15, RGBColor(0xD6, 0xD9, 0xF7), True)]])
...
bullets(s, [
    ...
    "没有就上亚马逊(多国站点),用 4–5 种排序一个个搜、一张张点开看",
...
steps = ["上传表格", "AI 解析归一", "AI 翻译关键词", "第三方数据检索", "AI 相似度分析",
...
cards = [
    ...
    ("AI 推荐可用链接", "电商翻译 → 第三方数据检索 → 相似度分析 → 三重核验 → 按要求择优,推荐含价格/销量/评论的可用对标。", "2"),
...
rows = [
    ...
    ("查找对标", "关键词精确匹配,差一个字就找不到", "语义检索 + 第三方数据,相似即命中"),
...
# ───────── S7 不用 RPA ─────────
s = slide()
header(s, "技术选型", "不用 RPA:第三方数据 API + AI")
bullets(s, [
    ("合法、稳定、可预算", "用第三方官方商用数据 API,不碰反爬、不踩验证码、无封号风险"),
    ("上线快、维护轻", "无需住宅代理与反爬对抗,合规清晰"),
    ("数据天生适配", "第三方数据自带价格历史 + 销量 + 评论,正好满足销量/评分/最便宜的选品"),
], t=Inches(2.1), gap=12)
...
bullets(s, [
    ...
    ("月度运营:约 ¥1,400 – ¥4,300", "数据订阅 + AI 调用 + 服务器(商标核验用官方公开数据,免费)"),
